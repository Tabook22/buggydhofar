  ...
  return (
    <section className="liability-waiver mt-8 rounded-2xl border border-white/10 bg-white/5 p-5 md:p-6">
      ...
      <div
        ...
        className="liability-waiver-document mt-4 max-h-72 overflow-y-auto rounded-xl border border-white/10 bg-black/30 p-4 text-sm leading-relaxed text-white/85 whitespace-pre-wrap"
      >
        {waiverText}
      </div>
      <label
        className={`mt-5 flex cursor-pointer items-start gap-3 rounded-xl border p-4 transition ${
          canAccept ? "border-white/10 bg-white/5 hover:bg-white/10" : "cursor-not-allowed border-white/5 bg-white/[0.02] opacity-60"
        }`}
      >
        <input
          type="checkbox"
      ...
      {!canAccept && (
        <p className="theme-alert-warning mt-2 text-xs text-amber-200/80">{t("booking.waiverFillDetails")}</p>
      )}
      {canAccept && !accepted && (
